...
웹메일 자동 로그인은 HTTPS 주소 또는 운영상 필요한 `http://webmail.reqm.co.kr`에서만
실행됩니다. HTTP 예외는 해당 호스트의 기본 포트에만 적용되며, 로그인 도중 다른 호스트로
이동하면 작업을 중단합니다. 필요한 경우 `REQM_WEBMAIL_URL`과
`REQM_WEBMAIL_INBOX_URL` 환경 변수로 검증된 HTTPS 주소를 지정할 수 있습니다.

위킵 반영을 선택한 출고 작업은 암호화된 로컬 작업 DB에 원본 출고 행과 출력 양식을
보관합니다. `송장번호 가져오기`에서 과거 작업을 다시 선택할 수 있고, 조회된 송장은
작업별로 누적됩니다. 전체 주문의 송장이 확인된 뒤에만 기존 출고 양식으로 최종 Excel을
한 번 저장하며, 저장된 파일은 기본 Excel 프로그램으로 자동 실행됩니다.
