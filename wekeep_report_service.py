...
def run_report() -> dict:
    ...
    with sync_playwright() as playwright:
        try:
            rows = collect_inventory_rows(playwright, list(selected), headless=True)
        except Exception as background_error:
            try:
                rows = collect_inventory_rows(playwright, list(selected), headless=False)
    ...
    matched_codes = {row["wekeep_code"].casefold() for row in matched}
    missing = [row for code, row in selected.items() if code not in matched_codes]
    if missing:
        missing_text = ", ".join(f"{row['item_name']} ({row['wekeep_code']})" for row in missing)
        raise RuntimeError(f"위킵에서 재고를 찾지 못한 선택 품목이 있습니다: {missing_text}")
